...
LOCAL_CHART_PATH="helm/manifest-storage-service-chart"
...
echo "--- Building local manifest-storage-service image inside of Minikube ---"
# Will have local image manifest-storage-service:latest in minikube
bash ./hack/build-local-manifest-storage-service-into-minikube.sh
...
target_name = 'manifest-storage-service-chart'
...
for line in lines:
    ...
    # If we are inside the manifest-storage-service-chart block, replace values
    if in_block:
...
kubectl rollout restart deployment -n $NAMESPACE -l app.kubernetes.io/name=manifest-storage-service-chart || true

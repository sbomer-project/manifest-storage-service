...
# Variables for minikube profile and manifest-storage-service
SBOM_SERVICE_IMAGE="manifest-storage-service:latest"
PROFILE="sbomer"
TAR_FILE="manifest-storage-service.tar"
...
echo "--- Exporting manifest-storage-service image to archive ---"
if [ -f "$TAR_FILE" ]; then
    rm "$TAR_FILE"
fi
podman save -o "$TAR_FILE" "$SBOM_SERVICE_IMAGE"

echo "--- Loading manifest-storage-service into Minikube ---"
